...
func (u *ClassController) AddAClass() {
	...
	if datas["responseNo"] == 0 {
		datas["responseNo"] = classObj.CreateAClass(mobilePhoneNumber,className,helper.StrToInt(schoolId))
	}
	//return
	u.jsonEcho(datas,u)
}
...
func (u *ClassController) GetAllClasses() {
	...
	datas["responseNo"] = u.checkSign(u)
	if datas["responseNo"] == 0 {
		datas["classList"] = classObj.GetSchoolClassInfo(helper.StrToInt((schoolId)))
	}
	//return
	u.jsonEcho(datas,u)
}